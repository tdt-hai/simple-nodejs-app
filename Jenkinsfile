pipeline {
    ...
    environment {
        // ── GitLab Registry ──────────────────────────────────────────────
        GITLAB_REGISTRY   = "gitsys.itmwg.vn:6666"
        GITLAB_PROJECT    = "142189/casestudy"          // đổi lại
        IMAGE_NAME        = "${GITLAB_REGISTRY}/${GITLAB_PROJECT}"
        ...
        // Credential ID lưu trong Jenkins (username = gitlab token, password = token value)
        REGISTRY_CREDS    = "gitlab-credentials"

        // ── App / Deploy ─────────────────────────────────────────────────
        APP_PORT          = "8080"
        CONTAINER_NAME    = "casestudy-app"
    }
    ...
    stages {
        ...
        // ── 4. IMAGE PUSH to GitLab Registry ─────────────────────────────
        stage("Image Push") {
            steps {
                ...
                withCredentials([usernamePassword(
                    ...
                    sh """
                        echo "${REG_PASS}" | docker login ${GITLAB_REGISTRY} \
                            --username "${REG_USER}" \
                            --password-stdin
